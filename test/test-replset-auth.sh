...
mongo localhost:27017/rs --quiet --eval "rs.slaveOk(); db.getCollectionNames()" | grep "not authorized" &> /dev/null
status=$?
if [[ "$status" == "0" ]]; then
...
mongo localhost:27017/rs -u auth -p auth --eval 'rs.slaveOk(); db.getCollectionNames()' &> /dev/null
status=$?
if [[ "$status" == "0" ]]; then
